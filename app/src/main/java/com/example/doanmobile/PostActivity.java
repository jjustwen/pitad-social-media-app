                ...
                public void onComplete(@NonNull Task<Uri> task)
                {
                    if (task.isSuccessful())
                    {
                        ...
                        db.collection("Posts").get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>()
                        {
                            ...
                            {
                                //Thêm 1 thông báo vào collection "Notifications"
                                db.collection("Notifications").get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>()
                                {
                                    @Override
                                    public void onComplete(@NonNull Task<QuerySnapshot> task)
                                    {
                                        Notification notification = new Notification(notifyID, currentUserID, "", "đã đăng bài viết mới", postID, task.getResult().size() + 1);
                                        db.collection("Notifications").document(notifyID).set(notification);
                                    }
                                });

                                //thêm 1 bài post vào danh sách bài post của user hiện tại
                                ...
                                //Thêm 1 bài post vào collection "Posts"
